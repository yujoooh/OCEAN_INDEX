		...
		for FC_AREA in FORECAST_AREA : 
			for DY in DATE :
				...
				for VAR in VARS :
					...
					globals()['NUM_{}'.format(VAR)] = LIST3_DY[VAR].count()/4 #[자료의 수(가중치 산정용) : 오전은 09, 12시 자료만 셀렉 되었으며, 이중 절반이 전체 여행에서 해당 변수가 활용되는 수]
					...
					if VAR == 'WARN1' or VAR == 'WARN2' : 
						...
						#globals()['WARN1_{}'.format('AM')] = '·'.join(WARN1_A) ; globals()['WARN1_{}'.format('PM')] = '·'.join(WARN1_P)
						globals()['WARN1_{}'.format('DY')] = '·'.join(WARN2_D)
						#globals()['WARN2_{}'.format('AM')] = '·'.join(WARN2_A) ; globals()['WARN2_{}'.format('PM')] = '·'.join(WARN2_P)
						globals()['WARN2_{}'.format('DY')] = '·'.join(WARN2_D)
					else :
				...
				if DY == today or DY == afterday1 or DY == afterday2 : TIME = ['DY']
