...
while ii <= INF_LEN-1 :
	...
	while jj <= 144:
		if os.path.isfile(INFILE_6) :
			if  jj <= 48 and NAME != '울릉도' :
				SST_S  = Statistic().min_max_ave('KHOA', 'daily', SST, MOHID_X, MOHID_Y, jj)
				CURRENT_S = Statistic().min_max_ave('KHOA', 'daily', CURRENT, MOHID_X, MOHID_Y, jj)
			else :
				SST_S  = Statistic().min_max_ave('KHOA', 'daily', SST2, ROMS_X, ROMS_Y, jj)
				CURRENT_S = Statistic().min_max_ave('KHOA', 'daily', CURRENT2, ROMS_X, ROMS_Y, jj)
		else : 
		...
		if jj == 0  : DAY1.append([STN, AREA, W_AREA, NAME, today, TODAY_MUL,         round(WAVE_S[0],1), round(WAVE_S[2],1), round(WAVE_S[1],1), round(CURRENT_S[0],1), round(CURRENT_S[2],1), round(CURRENT_S[1],1), int(round(SST_S[0])), int(round(SST_S[2])), int(round(SST_S[1]))])
		if jj == 24 : DAY2.append([STN, AREA, W_AREA, NAME, afterday1, AFTERDAY1_MUL, round(WAVE_S[0],1), round(WAVE_S[2],1), round(WAVE_S[1],1), round(CURRENT_S[0],1), round(CURRENT_S[2],1), round(CURRENT_S[1],1), int(round(SST_S[0])), int(round(SST_S[2])), int(round(SST_S[1]))])
		if jj == 48 : DAY3.append([STN, AREA, W_AREA, NAME, afterday2, AFTERDAY2_MUL, round(WAVE_S[0],1), round(WAVE_S[2],1), round(WAVE_S[1],1), round(CURRENT_S[0],1), round(CURRENT_S[2],1), round(CURRENT_S[1],1), int(round(SST_S[0])), int(round(SST_S[2])), int(round(SST_S[1]))])
		if jj == 72 : DAY4.append([STN, AREA, W_AREA, NAME, afterday3, AFTERDAY3_MUL, round(WAVE_S[0],1), round(WAVE_S[2],1), round(WAVE_S[1],1), round(CURRENT_S[0],1), round(CURRENT_S[2],1), round(CURRENT_S[1],1), int(round(SST_S[0])), int(round(SST_S[2])), int(round(SST_S[1]))])
		if jj == 96 : DAY5.append([STN, AREA, W_AREA, NAME, afterday4, AFTERDAY4_MUL, round(WAVE_S[0],1), round(WAVE_S[2],1), round(WAVE_S[1],1), round(CURRENT_S[0],1), round(CURRENT_S[2],1), round(CURRENT_S[1],1), int(round(SST_S[0])), int(round(SST_S[2])), int(round(SST_S[1]))])
		if jj == 120 : DAY6.append([STN, AREA, W_AREA, NAME, afterday5, AFTERDAY5_MUL, round(WAVE_S[0],1), round(WAVE_S[2],1), round(WAVE_S[1],1), round(CURRENT_S[0],1), round(CURRENT_S[2],1), round(CURRENT_S[1],1), int(round(SST_S[0])), int(round(SST_S[2])), int(round(SST_S[1]))])
		if jj == 144 : DAY7.append([STN, AREA, W_AREA, NAME, afterday6, AFTERDAY6_MUL, round(WAVE_S[0],1), round(WAVE_S[2],1), round(WAVE_S[1],1), round(CURRENT_S[0],1), round(CURRENT_S[2],1), round(CURRENT_S[1],1), int(round(SST_S[0])), int(round(SST_S[2])), int(round(SST_S[1]))])
...
while ii <= COM_LEN-1 :
	...
	TIDE_SCRE = IndexScore().tide_score(COM[ii][5])
	WAVE_SCRE = IndexScore().wave_score(round(float(COM[ii][8]),1))
	CURRENT_SCRE = IndexScore().current_score(round(COM[ii][11],1))
	SST_SCRE = IndexScore().sst_score(COM[ii][13])
	WARN_SCRE = IndexScore().warn_score(COM[ii][15])
	...
	COM[ii].insert(6,TIDE_SCRE) ; COM[ii].insert(10,WAVE_SCRE) ; COM[ii].insert(14,CURRENT_SCRE); COM[ii].insert(18,SST_SCRE) ; COM[ii].insert(20,WARN_SCRE) ;COM[ii].insert(21,TOTAL_SCRE1) ; COM[ii].insert(22,TOTAL_SCRE2) ; COM[ii].insert(23,QUOTIENT_SCRE1) ; COM[ii].insert(24,QUOTIENT_SCRE2)
	ii = ii + 1
...
with open(OUT_FNAME1,'w',encoding='utf8') as file:
	file.write('생산일,코드,권역,지역,예측일자,물때,물때점수,최소파고,평균파고,최대파고,파고점수,최소유속,평균유속,최대유속,유속점수,최소수온,평균수온,최대수온,수온점수,총점수,예보지수\n',)
	ii = 0
	while ii <= COM_LEN-1 :
		#[전체 데이터 Writing]
		file.write(f'{today},{COM[ii][0]},{COM[ii][1]},{COM[ii][3]},{COM[ii][4]},{COM[ii][5]},{COM[ii][6]},{COM[ii][7]:2.1f},{COM[ii][8]:2.1f},{COM[ii][9]:2.1f},{COM[ii][10]},{COM[ii][11]:2.1f},{COM[ii][12]:2.1f},{COM[ii][13]:2.1f},{COM[ii][14]},{COM[ii][15]},{COM[ii][16]},{COM[ii][17]},{COM[ii][18]},{COM[ii][21]:4.2f},{COM[ii][23]}\n')
		ii = ii + 1

OUT_FNAME2=dir1+'/SSEQ_INDEX_SERVICE_'+today+'.csv'
with open(OUT_FNAME2,'w',encoding='utf8') as file:
	file.write('코드,권역,지역,예측일자,물때,물때점수,최소파고,평균파고,최대파고,파고점수,최소유속,평균유속,최대유속,유속점수,최소수온,평균수온,최대수온,수온점수,특보,특보점수,총점수,예보지수\n',)
	ii = 0
	while ii <= COM_LEN-1 :
		#[WEB_~~_SCRE Format]
		file.write(f'{COM[ii][0]},{COM[ii][1]},{COM[ii][3]},{COM[ii][4]},{COM[ii][5]},{COM[ii][6]},{COM[ii][7]:2.1f},{COM[ii][8]:2.1f},{COM[ii][9]:2.1f},{COM[ii][10]:2.1f},{COM[ii][11]:2.1f},{COM[ii][12]:2.1f},{COM[ii][13]:2.1f},{COM[ii][14]},{COM[ii][15]},{COM[ii][16]},{COM[ii][17]},{COM[ii][18]},{COM[ii][19]},{COM[ii][20]},{COM[ii][22]:4.2f},{COM[ii][24]}\n')
		ii = ii + 1
