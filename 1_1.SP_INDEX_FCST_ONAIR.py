...
#[Info Data Read]=========================================================================================
POINT_INF = open('./Info/SP_Point_Info_ONAIR.csv','r',encoding='utf8')
...
while ii <= INF_LEN-1 :
	...
	while jj <= 144:
		...
		if jj == 0 : # today am/pm data 
			DAY1.append([STN, AREA, W_AREA, NAME, today, 'DY', TEMP_S[2], SST_S[2], WIND_S[1], WAVE_S[1]])
		elif jj == 24 :
			DAY2.append([STN, AREA, W_AREA, NAME, afterday1, 'DY', TEMP_S[2], SST_S[2], WIND_S[1], WAVE_S[1]])
		elif jj == 48 :
			DAY3.append([STN, AREA, W_AREA, NAME, afterday2, 'DY', TEMP_S[2], SST_S[2], WIND_S[1], WAVE_S[1]])
		elif jj == 72 :
...
while ii <= COM_LEN-1:
	if CITY_LEN == 0 :
		...
		while jj <= CITY_LEN-1:
			...
			if COM[ii][0] == STN and COM[ii][4] == DATE and DEV[2] == DEV2[2]:
				COM[ii].insert(10,(RAIN_AMT + float(DEV2[11])))
				if int(SKY) >= int(DEV2[10]) : COM[ii].insert(11,SKY)
				else : COM[ii].insert(11,DEV2[10])
			elif COM[ii][0] == STN and (COM[ii][4] == afterday3 or COM[ii][4] == afterday4 or COM[ii][4] == afterday5 or COM[ii][4] == afterday6) :
				COM[ii].insert(10,0)
				COM[ii].insert(11,1)
			jj = jj + 1
	ii = ii + 1
