...
from Function.tleq_function import IndexScore, TL_time_cal
from Function.statistic_function import Statistic

print("#TLEQ FCST_INDEX and SERVICE INDEX CREAT Start==========")
...
#[Info Data Read]=========================================================================================
POINT_INF = open('./Info/TL_Point_Info.csv','r',encoding='utf8')
INF = [str(INFO) for INFO in POINT_INF.read().split()]
INF_LEN = len(INF)

#[SD Split data read]=========================================================================
SDTIME_INF = open('./Info/TL_Time.csv','r',encoding='utf8')
SDTIME = [LST for LST in SDTIME_INF.read().split()]
SDTIME_LEN = len(SDTIME)

#[KMA CITY and WARNING data read]=========================================================================
if not os.path.exists(dir1+'/TL_CITY.csv'): # 동네예보 수집 에러에 따른 자료 미생성시 그냥 예측자료 사용
	print('CITY_FORECAST DATA Not found') ; CITY_LEN=0
else:
	CITY_INF = open(dir1+'/TL_CITY.csv','r',encoding='utf8')
...
for SDT_DATA in SDTIME :
	...
	if YMD == today :
		...
		TL_TIME = TL_time_cal().tl_time(YMD, START_TIME, END_TIME)

		ii = 1
		while ii <= INF_LEN-1 :
			DEV = INF[ii].split(',')
			STN = DEV[0] ; AREA = DEV[1] ; NAME = DEV[2] ; LON = DEV[3] ; LAT = DEV[4]
			WRF_X = int(DEV[7]) ; WRF_Y = int(DEV[8]) ; W_AREA = DEV[20] ; USE_YN=DEV[21]
			
			if SPLIT_NAME == NAME and USE_YN == 'Y' :
				WIND_S  = Statistic().min_max_ave('KHOA', 'daily', WIND, WRF_X, WRF_Y, 0)
				TEMP_S  = Statistic().min_max_ave('KHOA', 'daily', TEMP, WRF_X, WRF_Y, 0)
				DAY1.append([STN, AREA, W_AREA, NAME, today, TL_TIME[0], TL_TIME[1], TL_TIME[2], TL_TIME[3], int(round(TEMP_S[0])),  int(round(TEMP_S[2])), int(round(TEMP_S[1])), int(round(WIND_S[0])),int(round(WIND_S[2])), int(round(WIND_S[1]))])
			ii = ii + 1
		...
		while ii <= INF_LEN-1 :
			...
			if SPLIT_NAME == NAME:
				...
				DAY2.append([STN, AREA, W_AREA, NAME, afterday1, TL_TIME[0], TL_TIME[1], TL_TIME[2], TL_TIME[3], int(round(TEMP_S[0])),  int(round(TEMP_S[2])), int(round(TEMP_S[1])), int(round(WIND_S[0])),int(round(WIND_S[2])), int(round(WIND_S[1]))])
			ii = ii + 1
		...
		while ii <= INF_LEN-1 :
			...
			if SPLIT_NAME == NAME :
				...
				DAY3.append([STN, AREA, W_AREA, NAME, afterday2, TL_TIME[0], TL_TIME[1], TL_TIME[2], TL_TIME[3], int(round(TEMP_S[0])),  int(round(TEMP_S[2])), int(round(TEMP_S[1])), int(round(WIND_S[0])),int(round(WIND_S[2])), int(round(WIND_S[1]))])
			ii = ii + 1
	elif YMD == afterday3 :
		#SD_time_cal().split_time(YMD, START_TIME, END_TIME)
		TL_TIME = TL_time_cal().tl_time(YMD, START_TIME, END_TIME)
		...
		while ii <= INF_LEN-1 :
			...
			if SPLIT_NAME == NAME  :
				...
				DAY4.append([STN, AREA, W_AREA, NAME, afterday3, TL_TIME[0], TL_TIME[1], TL_TIME[2], TL_TIME[3], int(round(TEMP_S[0])),  int(round(TEMP_S[2])), int(round(TEMP_S[1])), int(round(WIND_S[0])),int(round(WIND_S[2])), int(round(WIND_S[1]))])
			ii = ii + 1
	elif YMD == afterday4 :
		#SD_time_cal().split_time(YMD, START_TIME, END_TIME)
		TL_TIME = TL_time_cal().tl_time(YMD, START_TIME, END_TIME)
		...
		while ii <= INF_LEN-1 :
			...
			if SPLIT_NAME == NAME :
				...
				DAY5.append([STN, AREA, W_AREA, NAME, afterday4, TL_TIME[0], TL_TIME[1], TL_TIME[2], TL_TIME[3], int(round(TEMP_S[0])),  int(round(TEMP_S[2])), int(round(TEMP_S[1])), int(round(WIND_S[0])),int(round(WIND_S[2])), int(round(WIND_S[1]))])
			ii = ii + 1
	elif YMD == afterday5 :
		#SD_time_cal().split_time(YMD, START_TIME, END_TIME)
		TL_TIME = TL_time_cal().tl_time(YMD, START_TIME, END_TIME)
		...
		while ii <= INF_LEN-1 :
			...
			if SPLIT_NAME == NAME:
				...
				DAY6.append([STN, AREA, W_AREA, NAME, afterday5, TL_TIME[0], TL_TIME[1], TL_TIME[2], TL_TIME[3], int(round(TEMP_S[0])),  int(round(TEMP_S[2])), int(round(TEMP_S[1])), int(round(WIND_S[0])),int(round(WIND_S[2])), int(round(WIND_S[1]))])
			ii = ii + 1
	elif YMD == afterday6 :
		#SD_time_cal().split_time(YMD, START_TIME, END_TIME)
		TL_TIME = TL_time_cal().tl_time(YMD, START_TIME, END_TIME)
		...
		while ii <= INF_LEN-1 :
			...
			if SPLIT_NAME == NAME :
				...
				DAY7.append([STN, AREA, W_AREA, NAME, afterday6, TL_TIME[0], TL_TIME[1], TL_TIME[2], TL_TIME[3], int(round(TEMP_S[0])),  int(round(TEMP_S[2])), int(round(TEMP_S[1])), int(round(WIND_S[0])),int(round(WIND_S[2])), int(round(WIND_S[1]))])
			ii = ii + 1
...
#[Output Data] ========================================================================================
OUT_FNAME1=dir1+'/TLEQ_INDEX_FCST_'+today+'.csv'
with open(OUT_FNAME1,'w',encoding='utf8') as file:
	file.write('생산일,코드,권역,지역,예측일자,갯벌열림시간,갯벌닫힘시간,체험시간,체험시간점수,최저기온,평균기온,최고기온,기온점수,최저풍속,평균풍속,최대풍속,풍속점수,날씨,날씨점수,총점수,예보지수\n',)
	ii = 0
	while ii <= COM_LEN-1 :
		#[전체 데이터 Writing]
		file.write(f'{today},{COM[ii][0]},{COM[ii][1]},{COM[ii][3]},{COM[ii][4]},{COM[ii][5]},{COM[ii][6]},{COM[ii][7]:3.2f},{COM[ii][8]},{COM[ii][10]},{COM[ii][11]},{COM[ii][12]},{COM[ii][13]},{COM[ii][14]},{COM[ii][15]},{COM[ii][16]},{COM[ii][17]},{COM[ii][20]},{COM[ii][21]},{COM[ii][24]:4.2f},{COM[ii][26]}\n')
		ii = ii + 1

OUT_FNAME2=dir1+'/TLEQ_INDEX_SERVICE_'+today+'.csv'
with open(OUT_FNAME2,'w',encoding='utf8') as file:
	file.write('생산일,코드,권역,지역,예측일자,갯벌열림시간,갯벌닫힘시간,체험시간,체험시간점수,최저기온,평균기온,최고기온,기온점수,최저풍속,평균풍속,최대풍속,풍속점수,날씨,날씨점수,강수량,강수점수,특보,특보점수,총점수,예보지수\n',)
	ii = 0
	while ii <= COM_LEN-1 :
		#[WEB_~~_SCRE Format]
		file.write(f'{today},{COM[ii][0]},{COM[ii][1]},{COM[ii][3]},{COM[ii][4]},{COM[ii][5]},{COM[ii][6]},{COM[ii][7]:3.2f},{COM[ii][8]},{COM[ii][10]},{COM[ii][11]},{COM[ii][12]},{COM[ii][13]},{COM[ii][14]},{COM[ii][15]},{COM[ii][16]},{COM[ii][17]},{COM[ii][20]},{COM[ii][21]},{COM[ii][18]},{COM[ii][19]},{COM[ii][22]},{COM[ii][23]},{COM[ii][25]:4.2f},{COM[ii][27]}\n')
		ii = ii + 1
		
	
print("#TLEQ FCST_INDEX and SERVICE INDEX CREAT Complete==========")
