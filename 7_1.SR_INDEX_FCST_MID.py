...
for i in range(len(POINT_df)):
    ...
    for date in COM['DATE'].unique().tolist():
        if date >= int(afterday3):
        #[WW3]
            mean_WHT = round(COM['WHT'][(COM['FCST_AREA'] == FCST_AREA) & (COM['NAME'] == NAME) & (COM['DATE'] == date) & (COM['HR'] <= 18) & (COM['HR'] >= 9) ].mean(),1)
            mean_WAVEX = COM['Wavex'][(COM['FCST_AREA'] == FCST_AREA) & (COM['NAME'] == NAME) & (COM['DATE'] == date) & (COM['HR'] <= 18) & (COM['HR'] >= 9) ].mean()
            mean_WAVEY = COM['Wavey'][(COM['FCST_AREA'] == FCST_AREA) & (COM['NAME'] == NAME) & (COM['DATE'] == date) & (COM['HR'] <= 18) & (COM['HR'] >= 9) ].mean()
            mean_WAVEDIR = round(np.rad2deg(np.arctan2(mean_WAVEX,mean_WAVEY)),0)
            #print(mean_WAVEDIR)
            if mean_WAVEDIR < 0:
            ...
            mean_RPEAK = round(COM['Rpeak'][(COM['FCST_AREA'] == FCST_AREA) & (COM['NAME'] == NAME) & (COM['DATE'] == date) & (COM['HR'] <= 18) & (COM['HR'] >= 9) ].mean(),1)
            #[WRF]
            mean_WINDS = round(COM['풍속'][(COM['FCST_AREA'] == FCST_AREA) & (COM['NAME'] == NAME) & (COM['DATE'] == date) & (COM['HR'] <= 18) & (COM['HR'] >= 9) ].mean(),1)
            mean_WINDDIR = COM['풍향'][(COM['FCST_AREA'] == FCST_AREA) & (COM['NAME'] == NAME) & (COM['DATE'] == date) & (COM['HR'] <= 18) & (COM['HR'] >= 9) ].mean()
            #[YES3K]
            mean_TEMP = round(COM['수온'][(COM['FCST_AREA'] == FCST_AREA) & (COM['NAME'] == NAME) & (COM['DATE'] == date) & (COM['HR'] <= 18) & (COM['HR'] >= 9) ].mean(),1)
...
COM_INDEX2.to_csv('./Result/'+today+'/'+today+'_SREQ_INDEX_FCST_MID.csv', encoding = 'utf8', index=False)
#COM_INDEX2.to_csv('./Result/'+today+'/'+today+'_SURFING_INDEX_FCST_mid_euckr.csv', encoding = 'euc-kr', index=False)
COM_INDEX.to_csv('./Result/'+today+'/'+today+'_SREQ_INDEX_SERVICE_MID.csv', encoding = 'utf8', index=False)
