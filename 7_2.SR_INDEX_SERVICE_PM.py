...
afterday1 = (datetime.strptime(today,"%Y%m%d") + timedelta(1)).strftime('%Y%m%d')
afterday2 = (datetime.strptime(today,"%Y%m%d") + timedelta(2)).strftime('%Y%m%d')
afterday3 = (datetime.strptime(today,"%Y%m%d") + timedelta(3)).strftime('%Y%m%d')
afterday4 = (datetime.strptime(today,"%Y%m%d") + timedelta(4)).strftime('%Y%m%d')
afterday5 = (datetime.strptime(today,"%Y%m%d") + timedelta(5)).strftime('%Y%m%d')
afterday6 = (datetime.strptime(today,"%Y%m%d") + timedelta(6)).strftime('%Y%m%d')
...
SURFING_INF = open(dir1+'/'+ today+ '_SREQ_INDEX_SERVICE_PM.csv',encoding='utf8')
...
#'코드,권역,지역,예측일자,어종,어종타입,물때,물때점수,최소파고,평균파고,최대파고,파고점수,최소수온,평균수온,최대수온,수온점수,최저기온,평균기온,최대기온,기온점수,최소풍속,평균풍속,최대풍속,풍속점수,특보,특보점수,총점수
	CODE = DEV[20]; Area = DEV[1] ; Point = DEV[2] ; FDATE = DEV[3]
	#MIN_WHT = DEV[8]; MAX_WHT = DEV[10]
	AVE_WHT = DEV[4]; AVE_WAVEDIR = DEV[6]; AVE_R_WAVEDIR = DEV[7];\
	AVE_RPEAK = float(DEV[9]); AVE_WIND = float(DEV[11]); AVE_WINDDIR = float(DEV[12]); AVE_SST = float(DEV[14])  
	...
	#파향 들어올 경우 여기 수정해야함
	fdata.append([Point,FDATE, AVE_WHT, AVE_WAVEDIR, AVE_R_WAVEDIR, AVE_SST, AVE_RPEAK])
	rawdata.append([CODE,Area,Point, FDATE, AVE_WHT, AVE_WAVEDIR, AVE_R_WAVEDIR, AVE_RPEAK, AVE_WIND, AVE_WINDDIR, AVE_SST, WARN])
	ii += 1

odata = []
if hr < 12: # 오전자료 읽기
	AM_DATA = open(dir1+'/SR_POINT_OBS_PM.csv','r',encoding='utf8')
...
for kk in range(len(odata)):
    for nn in range(len(POINT_df)):
        if odata[kk][0] == POINT_df['NAME'][nn]:
            #print(odata[kk][0])
            odata[kk].insert(3, float(odata[kk][2]) - POINT_df['BEACH_DIR'][nn])
...
# 						apply_SST = round(float(fdata[kk][3]),2)
			...
			#print(fdata[kk][1])
			apply1.append([Diff_Point,fdata[kk][1],obj_WHT,apply_WHT, obj_WDIR, apply_WDIR, obj_RWDIR, apply_RWDIR, obj_SST, apply_SST, obj_RPEAK, apply_RPEAK])
...
while ii < len(apply1) :
	...
	TOTAL_SCRE2 = IndexScore().total_score(WAVE_SCRE, RPEAK_SCRE, R_WAVEDIR_SCRE, WIND_SCRE, TEMP_SCRE, WARN_SCRE)[1]
	#예보지수
...
COM_INDEX.columns = ['산출날짜','코드','구분','지역','예측날짜','유의파고','파고점수','파향', '상대파향','파향점수', '파주기','파주기점수','풍속','풍향','풍속점수','수온','수온점수','특보','특보점수','총점수','예보지수']
#OUT_FNAME=dir1+'/SURFING_INDEX_SERVICE_last_PM'+today+'.csv'
OUT_FNAME=dir1+'/'+today+'_SREQ_INDEX_SERVICE_PM.csv'
