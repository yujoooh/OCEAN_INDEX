...
while ii < SKEQ_LEN:
	...
	MIN_WIND = DEV[14] ; AVE_WIND = DEV[15] ; MAX_WIND = DEV[16]
	fdata.append([Route,FDATE,FAMPM,MIN_WHT, AVE_WHT, MAX_WHT])
	rawdata.append([ii,Code,Route,Point, SHIP_Type, FDATE, FAMPM, SAIL_HR, Ton,MIN_WHT, AVE_WHT, MAX_WHT, MIN_WIND, AVE_WIND, MAX_WIND]) 
	ii += 1
#print(fdata)
...
kk = 0
apply1 = []
#while kk < int(AM_LEN*4):
#print((SKEQ_LEN-1)*2/3)
while kk < int((SKEQ_LEN-1)*2/7):
	mm = 0
	ll = 0
	while mm < AM_LEN:
...
#            if fdata[kk][1]== '2018-10-10':
#kk = int(AM_LEN*4)
#print(AM_LEN, AM_LEN*4, AM_LEN*6)

while kk < int((SKEQ_LEN-1)*3/7):
    mm = 0
    while mm < AM_LEN:
...
#	print(MIN_WHT, AVE_WHT, apply1[ii][3])
	...
	COM.append([rawdata[ii][1],rawdata[ii][2],rawdata[ii][3],rawdata[ii][4],rawdata[ii][5],rawdata[ii][6],rawdata[ii][7],SAIL_SCRE,rawdata[ii][8],TON_SCRE,MIN_WHT, AVE_WHT,apply1[ii][3], WAVE_SCRE,rawdata[ii][12],rawdata[ii][13],rawdata[ii][14],WIND_SCRE,round(TOTAL_SCRE1,2),QUOTIENT_SCRE1])
	ii = ii + 1
...
with open(OUT_FNAME,'w',encoding='utf8') as file:
	...
	while ii < len(COM) :
		#[전체 데이터 Writing]
		file.write(f'{today},{COM[ii][0]},{COM[ii][1]},{COM[ii][2]},{COM[ii][3]},{COM[ii][4]},{COM[ii][5]},{COM[ii][6]},{COM[ii][7]},{COM[ii][8]},{COM[ii][9]},{COM[ii][10]},{COM[ii][11]},{COM[ii][12]},{COM[ii][13]},{COM[ii][14]},{COM[ii][15]},{COM[ii][16]},{COM[ii][17]},{COM[ii][18]},{COM[ii][19]}\n')
		ii = ii + 1
