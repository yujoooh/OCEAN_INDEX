...
while ii < COM_TIME_LEN : 
	...
	if CITY_LEN == 0 :
		...
		COM_TIME[ii].insert(15,float(0)) # RAIN PROBABILITY
		COM_TIME[ii].insert(16,float(0)) # RELATIVE HUMEDITY
		COM_TIME[ii].insert(17,'-')    # SEA_WARNING
	else :
		jj = 1
		while jj < CITY_LEN :
			DEV = CITY[jj].split(',')
			...
			if COM_TIME[ii][0] == STN and COM_TIME[ii][4] == DATE and COM_TIME[ii][5] == str(HOUR):
				...
				COM_TIME[ii].insert(15,float(RAIN_PROB)) # RAIN PROBABILITY
				COM_TIME[ii].insert(16,float(REL_HU))    # RELATIVE HUMEDITY
				COM_TIME[ii].insert(17,'-')       # SEA_WARNING
				break
			jj = jj + 1
	ii = ii + 1
...
COM_TIME_DF.columns = ['STN','AREA','SEA_WARN','NAME','DATE','HR','TEMP','WSPD','WDIR','SST','WAVE','CURRENT','MUL','SKY','RAIN','RAIN_PROB','REL_HU','WARN1']
#print(COM_TIME_DF)
...
for STN in STATION : 
	...
	for DY in DATE :
		...
		for HR in HOUR :
			...
			SEA_WARN = list(set(LIST2['SEA_WARN'].values.tolist())) ; SEA_WARN = SEA_WARN[0]
			...
			FINAL.append([STN, AREA, SEA_WARN, NAME, DY, HR, TEMP_MAX, WSPD_MAX, WDIR_MAX, SST_MAX, WAVE_MAX, CURRENT_MAX, MUL_MAX, SKY_MAX, RAIN_MAX, RAIN_PROB_MAX, REL_HU_MAX, WARN1_COM])
	#ii = ii + 1
FINAL_DF = pd.DataFrame(FINAL)
FINAL_DF.columns = ['STN','AREA','SEA_WARN','NAME','DATE','HR','TEMP','WSPD','WDIR','SST','WAVE','CURRENT','MUL','SKY','RAIN','RAIN_PROB','REL_HU','WARN1']
