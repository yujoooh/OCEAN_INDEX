...
print("#SPEQ Timeseries CREAT Start==========")
...
if not os.path.exists(dir1+'/SP_CITY_time.csv'): # 동네예보 수집 에러에 따른 자료 미생성시 그냥 예측자료 사용
	print('CITY_FORECAST DATA Not found') ; CITY_LEN=0
else:
	CITY_INF = open(dir1+'/SP_CITY_time.csv','r',encoding='utf8')
	...
	if CITY_LEN == 0 :
		...
		while jj < CITY_LEN :
			...
			if COM_TIME[ii][0] == STN and COM_TIME[ii][4] == DATE and COM_TIME[ii][5] == str(HOUR):
				...
				COM_TIME[ii].insert(15,float(RAIN_ACC))  # RAIN AMOUNT_ACCUMULATE(00시~05시, 06시~08시, 09시~12시, 13시~18시, 19시~23시 / 누력자료 사용 시 오전은 12시(09~12 최종 누적값), 오후는 18시(13~18최종 누적값) 자료 활용해야함)
...
#[Output Data] ========================================================================================
OUT_FNAME1 = dir1+'/SP_TIMESERIES_'+today+'.csv'   # 시계열
COM_TIME_DF.to_csv(OUT_FNAME1,mode='w',encoding='utf8', index=False)

print("#SPEQ Timeseries CREAT Complete==========")
