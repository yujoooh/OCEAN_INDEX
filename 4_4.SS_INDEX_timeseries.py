...
print("#SSEQ Timeseries CREAT Start==========")
...
if not os.path.exists(dir1+'/SS_CITY_time.csv'): # 동네예보 수집 에러에 따른 자료 미생성시 그냥 예측자료 사용
	print('CITY_FORECAST DATA Not found') ; CITY_LEN=0
else:
	CITY_INF = open(dir1+'/SS_CITY_time.csv','r',encoding='utf8')
...
while ii < COM_TIME_LEN : 
	if CITY_LEN == 0 :
		...
		while jj < CITY_LEN :
			DEV = CITY[jj].split(',')
			STN = DEV[0] ; NAME = DEV[1] ; DATE = DEV[2] ; HOUR = DEV[3]; SKY = DEV[6] ; RAIN_AMT = DEV[7] ; RAIN_AMT_ACC = DEV[8] ;RAIN_PROB = DEV[9] ; REL_HU = DEV[10]
...
#[Output Data] ========================================================================================
OUT_FNAME1 = dir1+'/SS_TIMESERIES_'+today+'.csv'   # 시계열
COM_TIME_DF.to_csv(OUT_FNAME1,mode='w',encoding='utf8', index=False)

print("#SSEQ Timeseries CREAT Complete==========")
