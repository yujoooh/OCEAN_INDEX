...
while ii <= INF_LEN-1 :
	...
	RWW3_X = int(DEV[14]) ;  RWW3_Y = int(DEV[48]) ; MOHID_X = int(DEV[16]) ; MOHID_Y = int(DEV[17]) ; W_AREA = DEV[20]

	jj = 0 ; jjj = 0
	while jj <= 144:
		WIND_S = Statistic().min_max_ave('KHOA', 'ampm', WIND, WRF_X, WRF_Y, jj)
		TEMP_S = Statistic().min_max_ave('KHOA', 'ampm', TEMP, WRF_X, WRF_Y, jj)
		if os.path.isfile(INFILE_6):
			if jj <= 48: #0~2day
				SST_S  = Statistic().min_max_ave('KHOA', 'ampm', SST, MOHID_X, MOHID_Y, jj)
			else :       #3~6days
				SST_S  = Statistic().min_max_ave('KHOA', 'ampm', SST2, ROMS_X, ROMS_Y, jj)	
		else : 
			SST_S  = Statistic().min_max_ave('KHOA', 'ampm', SST, ROMS_X, ROMS_Y, jj)
		if os.path.isfile(INFILE_3):
			if jj <= 48 :
				jjj = int(jj / 3) #3시간 자료
		...
		if jj == 0 : # today am/pm data 
			DAY1.append([STN, AREA, W_AREA, NAME, today, 'AM', TEMP_S[2], SST_S[2], WIND_S[1], WAVE_S[1]])
			DAY1.append([STN, AREA, W_AREA, NAME, today, 'PM', TEMP_S[5], SST_S[5], WIND_S[4], WAVE_S[4]])
			#print(WAVE_S[1],WAVE_S[4])
		elif jj == 24 :
			DAY2.append([STN, AREA, W_AREA, NAME, afterday1, 'AM', TEMP_S[2], SST_S[2], WIND_S[1], WAVE_S[1]])
			DAY2.append([STN, AREA, W_AREA, NAME, afterday1, 'PM', TEMP_S[5], SST_S[5], WIND_S[4], WAVE_S[4]])
		elif jj == 48 :
			DAY3.append([STN, AREA, W_AREA, NAME, afterday2, 'AM', TEMP_S[2], SST_S[2], WIND_S[1], WAVE_S[1]])
			DAY3.append([STN, AREA, W_AREA, NAME, afterday2, 'PM', TEMP_S[5], SST_S[5], WIND_S[4], WAVE_S[4]])
		elif jj == 72 :
			DAY4.append([STN, AREA, W_AREA, NAME, afterday3, 'AM', TEMP_S[2], SST_S[2], WIND_S[1], WAVE_S[1]])
			DAY4.append([STN, AREA, W_AREA, NAME, afterday3, 'PM', TEMP_S[5], SST_S[5], WIND_S[4], WAVE_S[4]])
		elif jj == 96 :
			DAY5.append([STN, AREA, W_AREA, NAME, afterday4, 'AM', TEMP_S[2], SST_S[2], WIND_S[1], WAVE_S[1]])
			DAY5.append([STN, AREA, W_AREA, NAME, afterday4, 'PM', TEMP_S[5], SST_S[5], WIND_S[4], WAVE_S[4]])
		elif jj == 120 :
			DAY6.append([STN, AREA, W_AREA, NAME, afterday5, 'AM', TEMP_S[2], SST_S[2], WIND_S[1], WAVE_S[1]])
			DAY6.append([STN, AREA, W_AREA, NAME, afterday5, 'PM', TEMP_S[5], SST_S[5], WIND_S[4], WAVE_S[4]])
		elif jj == 144 :
			DAY7.append([STN, AREA, W_AREA, NAME, afterday6, 'AM', TEMP_S[2], SST_S[2], WIND_S[1], WAVE_S[1]])
			DAY7.append([STN, AREA, W_AREA, NAME, afterday6, 'PM', TEMP_S[5], SST_S[5], WIND_S[4], WAVE_S[4]])
...
while ii <= COM_LEN-1:
	...
	while jj <= WARN_LEN-1:
		...
		if int(COM[ii][4]) == int(WARN_SDAY) and COM[ii][5] == WARN_SHR and COM[ii][2] == WARN_AREA:
			if COM[ii][4] == today or COM[ii][4] == afterday1 or COM[ii][4] == afterday2:
				COM[ii].insert(12,WARN_TYPE)
			else :
				COM[ii].insert(12,'-')
			jj = WARN_LEN
		elif int(COM[ii][4]) >= int(WARN_SDAY) and COM[ii][2] == WARN_AREA:
			if COM[ii][4] == today or COM[ii][4] == afterday1 or COM[ii][4] == afterday2:
				COM[ii].insert(12,WARN_TYPE)
			else :
				COM[ii].insert(12,'-')
...
while ii <= COM_LEN-1 :
	...
	WAVE_SCRE = IndexScore().wave_score(round(COM[ii][9],1))
	...
	TOTAL_SCRE2 = IndexScore().total_score(TEMP_SCRE, round(COM[ii][7],1), SST_SCRE, WIND_SCRE, WAVE_SCRE, RAIN_SCRE, WARN_SCRE)[1] # service score(rain, warning)QUOTIENT_SCRE1 = IndexScore().quotient_score(TOTAL_SCRE1)
...
with open(OUT_FNAME1,'w',encoding='utf8') as file:
	file.write('생산일,코드,해수욕장,예측년월일,시간,평균기온,기온점수,평균수온,수온점수,최대풍속,풍속점수,최대파고,파고점수,총점수,예보지수\n',)
	ii = 0
	while ii <= COM_LEN-1 :
		#[전체 데이터 Writing]
		file.write(f'{today},{COM[ii][0]},{COM[ii][3]},{COM[ii][4]},{COM[ii][5]},{COM[ii][6]:3.1f},{COM[ii][7]},{COM[ii][8]:3.1f},{COM[ii][9]},{COM[ii][10]:3.1f},{COM[ii][11]},{COM[ii][12]:3.1f},{COM[ii][13]},{COM[ii][19]:4.2f},{COM[ii][21]}\n')
		ii = ii + 1
