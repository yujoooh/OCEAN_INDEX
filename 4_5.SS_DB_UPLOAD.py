...
while ii <= len(content1)-1 :
	...
	print(CODE, YMD, TIDE, TIDE_SCORE, MIN_WHT, AVE_WHT, MAX_WHT, WAVE_SCORE, MIN_CSPD, AVE_CSPD, MAX_CSPD, CSPD_SCORE, MIN_SST, AVE_SST, MAX_SST, SST_SCORE, WARN, WARN_SCORE, TOTAL)
	
	sqlStr  = f"merge into WEB_SSEQ_SCRE@svclink.nori.go.kr    "
	#sqlStr  = f"merge into WEB_SSEQ_SCRE    "
	sqlStr += f"using dual  "
	sqlStr += f"on (SS_CODE='{CODE}' and PRED_DATE='{YR}-{MN}-{DY}')  "
	...
	sqlStr += f"insert (SS_CODE, PRED_DATE, TIDE_TIME, TE_SCORE, MIN_WAVE_HEIGHT, AVG_WAVE_HEIGHT, MAX_WAVE_HEIGHT, WH_SCORE, MIN_CURRENT_SPEED, AVG_CURRENT_SPEED, MAX_CURRENT_SPEED, CS_SCORE, MIN_WATER_TEMP, AVG_WATER_TEMP, MAX_WATER_TEMP, WT_SCORE, TOTAL_SCORE, WARN, WARN_SCORE)  "
	sqlStr += f"values ('{CODE}', '{YR}-{MN}-{DY}', {TIDE}, {TIDE_SCORE}, {MIN_WHT}, {AVE_WHT}, {MAX_WHT}, {WAVE_SCORE}, {MIN_CSPD}, {AVE_CSPD}, {MAX_CSPD}, {CSPD_SCORE}, {MIN_SST}, {AVE_SST}, {MAX_SST}, {SST_SCORE}, {TOTAL}, '{WARN}', {WARN_SCORE})  "
