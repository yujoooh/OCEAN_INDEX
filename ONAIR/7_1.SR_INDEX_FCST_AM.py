...
for i in range(len(POINT_df)):
    ...
    for date in COM['DATE'].unique().tolist():
        #[WW3]
        mean_WHT = round(COM['WHT'][(COM['FCST_AREA'] == FCST_AREA) & (COM['NAME'] == NAME) & (COM['DATE'] == date) & (COM['HR'] <= 12) & (COM['HR'] >= 9) ].mean(),1)
        mean_WAVEX = COM['Wavex'][(COM['FCST_AREA'] == FCST_AREA) & (COM['NAME'] == NAME) & (COM['DATE'] == date) & (COM['HR'] <= 12) & (COM['HR'] >= 9) ].mean()
        mean_WAVEY = COM['Wavey'][(COM['FCST_AREA'] == FCST_AREA) & (COM['NAME'] == NAME) & (COM['DATE'] == date) & (COM['HR'] <= 12) & (COM['HR'] >= 9) ].mean()
        ...
        mean_RPEAK = round(COM['Rpeak'][(COM['FCST_AREA'] == FCST_AREA) & (COM['NAME'] == NAME) & (COM['DATE'] == date) & (COM['HR'] <= 12) & (COM['HR'] >= 9) ].mean(),1)
        #[WRF]
        mean_WINDS = round(COM['풍속'][(COM['FCST_AREA'] == FCST_AREA) & (COM['NAME'] == NAME) & (COM['DATE'] == date) & (COM['HR'] <= 12) & (COM['HR'] >= 9) ].mean(),1)
        mean_WINDDIR = COM['풍향'][(COM['FCST_AREA'] == FCST_AREA) & (COM['NAME'] == NAME) & (COM['DATE'] == date) & (COM['HR'] <= 12) & (COM['HR'] >= 9) ].mean()
        #[YES3K]
        mean_TEMP = round(COM['수온'][(COM['FCST_AREA'] == FCST_AREA) & (COM['NAME'] == NAME) & (COM['DATE'] == date) & (COM['HR'] <= 12) & (COM['HR'] >= 9) ].mean(),1)
        ...
        print(CODE, NAME)
        #20210628
        row = [FCST_AREA, NAME, date, round(mean_WHT,1), WAVE_SCRE, round(mean_WAVEDIR), round(mean_R_WAVEDIR), WAVEDIR_SCRE, round(mean_RPEAK,1), RPEAK_SCRE, round(mean_WINDS,1), round(mean_WINDDIR,0), WIND_SCRE, round(mean_TEMP,1), TEMP_SCRE, warning, WARN_SCRE, TOTAL_SCRE2,  QUOTIENT_SCRE2, CODE]
        row2 = [FCST_AREA, NAME, date, round(mean_WHT,1), WAVE_SCRE, round(mean_WAVEDIR), round(mean_R_WAVEDIR), WAVEDIR_SCRE, round(mean_RPEAK,1), RPEAK_SCRE, round(mean_WINDS,1), round(mean_WINDDIR,0), WIND_SCRE, round(mean_TEMP,1), TEMP_SCRE, TOTAL_SCRE1,  QUOTIENT_SCRE1, CODE]
...
COM_INDEX2.to_csv('./Result/'+today+'/'+today+'_SREQ_INDEX_FCST_AM.csv', encoding = 'utf8', index=False)
#COM_INDEX2.to_csv('./Result/'+today+'/'+today+'_SURFING_INDEX_FCST_AM_euckr.csv', encoding = 'euc-kr', index=False)
COM_INDEX.to_csv('./Result/'+today+'/'+today+'_SREQ_INDEX_SERVICE_AM.csv', encoding = 'utf8', index=False)
#COM_INDEX.to_csv('./Result/'+today+'/'+today+'_SURFING_INDEX_SERVICE_AM_euckr.csv', encoding = 'euc-kr', index=False)
#최종실행시간 체크
