...
#[Manual date set1]
today  = str(date(2021,8,9).strftime('%Y%m%d'))
yesterday = str((date(2021,8,9) - timedelta(1)).strftime('%Y%m%d'))
afterday1 = (date(2021,8,9) + timedelta(1)).strftime('%Y%m%d')
afterday2 = (date(2021,8,9) + timedelta(2)).strftime('%Y%m%d')
afterday3 = (date(2021,8,9) + timedelta(3)).strftime('%Y%m%d')
afterday4 = (date(2021,8,9) + timedelta(4)).strftime('%Y%m%d')
afterday5 = (date(2021,8,9) + timedelta(5)).strftime('%Y%m%d')
afterday6 = (date(2021,8,9) + timedelta(6)).strftime('%Y%m%d')
...
FORECAST_AREA = ['JEJU','BUSAN','YEOSU']
#FORECAST_AREA = ['YEOSU']
		...
		if CITY_LEN == 0 :
			...
			while jj < CITY_LEN :
				...
				if COM_TIME[ii][1] == STN and COM_TIME[ii][6] == DATE and COM_TIME[ii][7] == str(HOUR):
					COM_TIME[ii].insert(15,float(SKY))      # SKY [맑음:1, 구름많음:3, 흐림:4]
					COM_TIME[ii].insert(16,float(RAIN_AMT))  # RAIN AMOUNT
					COM_TIME[ii].insert(17,float(RAIN_PROB)) # RAIN PROBABILITY
					COM_TIME[ii].insert(18,float(REL_HU))    # RELATIVE HUMEDITY
					COM_TIME[ii].insert(19,'-')       # SEA_WARNING
					COM_TIME[ii].insert(20,'-')       # LAND_WARNING
					break
				jj = jj + 1
